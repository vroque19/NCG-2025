...
void print_tower_state(const char *name, tower_stack *tower) {
  printf("  %s (Rings: %u, Top Index: %d): ", name, tower->ring_count,
         tower->top_idx);
...
void test_stack_init(tower_stack *tower) {
  ...
  tower->top_idx = -1;
  assert_true(is_empty_tower(tower),
              "TC1.1: Stack 0 is empty after initialization.");
  assert_false(is_full_tower(tower),
               "TC1.2: Tower is not full after initialization");
  assert_equal_int(tower->top_idx, -1,
                   "TC1.3: Top index is -1 after initialization");
  assert_equal_uint8((int)tower->ring_count, 0,
                     "TC1.4: Ring count is 0 after initialization.");
...
void test_stack_push(tower_stack *tower) {
  ...
  push_tower(tower, 3);
  assert_equal_uint8(tower->rings[tower->top_idx], 3,
                     "TC2.1: Pushed 3 to tower");
  push_tower(tower, 2);
  assert_equal_uint8(tower->rings[tower->top_idx], 2,
                     "TC2.2: Pushed 2 to tower");
  push_tower(tower, 1);
  assert_equal_uint8(tower->rings[tower->top_idx], 1,
                     "TC2.3: Pushed 1 to tower");
  print_tower_state("test tower: ", tower);
}
...
void test_stack_pop(tower_stack *tower) {
  ...
  assert_equal_uint8(tower->top_idx, 1,
                     "TC3.2: Top index is 1 after popping one ring.");
  assert_equal_uint8(tower->ring_count, 2,
                     "TC3.3: ring count is 2 after popping one ring.");
  print_tower_state("test tower", tower);

  popped_ring = pop_tower(tower);
  assert_equal_uint8(popped_ring, 2, "TC3.4: Popped ring is size 2.");
  assert_equal_uint8(tower->top_idx, 0,
                     "TC3.5: Top index is 0 after popping two rings.");
  assert_equal_uint8(tower->ring_count, 1,
                     "TC3.6: ring count is 1 after popping two rings.");
  print_tower_state("test tower", tower);

  popped_ring = pop_tower(tower);
  assert_equal_uint8(popped_ring, 3, "TC3.7: Popped ring is size 3.");
  assert_true(is_empty_tower(tower),
              "TC3.8: Stack is empty after popping all rings.");
  assert_equal_uint8(tower->top_idx, -1,
                     "TC3.9: Top index is -1 after popping all rings.");
  assert_equal_uint8(tower->ring_count, 0,
                     "TC3.10: ring count is 0 after popping all rings.");
  print_tower_state("test tower", tower);
}
...
void test_stack_overflow(tower_stack *tower) {
  ...
  assert_true(is_full_tower(tower),
              "TC4.1: Stack is full after pushing MAX_RINGS.");
...
void test_stack_underflow(tower_stack *tower) {
  ...
  assert_equal_uint8(popped, -1,
                     "TC5.2: Popping from empty stack returns error.");
  print_tower_state("test tower", tower);
}
...
void test_stack_get_id(tower_stack *tower) {
  ...
  init_stack(tower);
  tower->tower_id = 42;
  assert_equal_uint8(tower->tower_id, 42,
                     "TC6.1: Tower ID is set and retrieved correctly.");
  print_tower_state("test tower", tower);
}
...
void test_stack_ring_count(tower_stack *tower) {
  ...
  init_stack(tower);
  assert_equal_uint8(tower->ring_count, 0,
                     "TC8.1: Ring count is 0 after init.");
  push_tower(tower, 1);
  assert_equal_uint8(tower->ring_count, 1,
                     "TC8.2: Ring count is 1 after one push.");
  push_tower(tower, 2);
  assert_equal_uint8(tower->ring_count, 2,
                     "TC8.3: Ring count is 2 after two pushes.");
  pop_tower(tower);
  assert_equal_uint8(tower->ring_count, 1,
                     "TC8.4: Ring count is 1 after one pop.");
  pop_tower(tower);
  assert_equal_uint8(tower->ring_count, 0,
                     "TC8.5: Ring count is 0 after popping all.");
  print_tower_state("test tower", tower);
}
