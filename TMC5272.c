...
void tmc5272_configEmergencyStop(uint16_t icID, uint8_t motor, uint8_t isEnabled)
{
	if(motor == 0)
	{
		tmc5272_fieldWrite(icID, TMC5272_GCONF_M0_STOP_ENABLE_FIELD, isEnabled);
	}
	else
	{
		tmc5272_fieldWrite(icID, TMC5272_GCONF_M1_STOP_ENABLE_FIELD, isEnabled);
	}
}
...
void tmc5272_tricoder_init(uint16_t icID, uint8_t motor, int32_t encoder_value)
{
	...
	tmc5272_fieldWrite(icID, TMC5272_GCONF_M1_EN_PWM_MODE_FIELD, 1);

	// 0x0: Disable external encoder inputs. Motor must be at standstill.
	tmc5272_fieldWrite(icID, TMC5272_ENCMODE_NBEMF_ABN_SEL_FIELD(motor), 0);
	// 0x2: Passive braking
	tmc5272_fieldWrite(icID, TMC5272_PWMCONF_FREEWHEEL_FIELD(motor), 2);

	// 0x0: Lowest standstill (hold) current at 1/32 full-scale. 
	// Must set to 0 to choose freewheel / coil short circuit for standstill.
	tmc5272_fieldWrite(icID, TMC5272_IHOLD_IRUN_IHOLD_FIELD(motor), 0);

	// 0x0: TOFF off time. NCLK = 24 + 32 * TOFF. 0 = Driver disable, all bridges off.
	// tmc5272_fieldWrite(icID, TMC5272_CHOPCONF_TOFF_FIELD(motor), 0);
	// Note: The above is directed from the datasheet; however, TMCL-IDE sets the following:
	tmc5272_fieldWrite(icID, TMC5272_CHOPCONF_TOFF_FIELD(motor), 0x3);

	// 0x00108000: All defaults. This covers qsc_enc_en that datasheet instructs to set to 1.
	tmc5272_writeRegister(icID, TMC5272_ENCMODE(motor), 0x00108000, NULL); 
	...
	tmc5272_fieldWrite(icID, TMC5272_ENC_CONST_FIELD(motor), 256<<16);
...
{
	tmc5272_fieldWrite(icID, TMC5272_ENCMODE_BEMF_HYST_FIELD(motor), hysteresis);
}
...
{
	return (int32_t)tmc5272_fieldRead(icID, TMC5272_X_ENC_FIELD(motor));
}